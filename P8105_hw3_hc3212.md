...
According to the result, 3 states (MA NJ PA) were observed at 7 or more
locations in 2002, while 11 states (CA FL MA MD NC NE NJ NY OH TX WA)
...
brfss_df %>% 
  filter(year %in% c("2006", "2010"),
         locationabbr == "NY") %>%
  group_by(year) %>% 
  ggplot(aes(data_value, fill = year)) +
...
``` r
accel_df %>% 
  mutate(activity_hour = floor((as.numeric(activity_minute)-1))/60) %>% 
  ggplot(aes(x = activity_hour, y = activity_count, color = day)) + 
  geom_line(alpha = 0.5) +
  labs(
